...
public abstract class AsyncRequestsClient extends AsyncClient {

    static final Logger logger = LoggerFactory.getLogger(AsyncRequestsClient.class);
    ...
    public AsyncRequestsClient(ApiContext apiContext) {
        super(apiContext);
    }
    ...
    public CompletableFuture<OperationResult> sigKey(String account) {
        try {
            ...
            logger.error("Failed sig key request", e);
            return failure(e);
        }
    }
    ...
    public CompletableFuture<OperationResult> rsaKey(String account) {
        try {
            ...
            logger.error("Failed rsa key request", e);
            return failure(e);
        }
    }
    ...
    public CompletableFuture<OperationResult> blsKey(String account) {
        try {
            ...
            logger.error("Failed bls key request", e);
            return failure(e);
        }
    }
    ...
    public CompletableFuture<OperationResult> doWrite(Session session, String scope, Records records, WriteOptions options) {
        try {
            ...
            if (records.getSerialization() != null) {
                ...
                //  Using isAllowLocalBatching as peer replication has this set to false and if we have serialization deferred on another thread it hurts performance when server replicates because of records structures leak
            ...
            sendRequest(requestType, request, true, session, null, null).whenComplete((data, e) -> {
                if (e != null) {
                    ...
                    try {
                        ...
                        logger.error("Failed task complete", ex);
                        reply.complete(null);
                    }
                }
            });
    ...
    public CompletableFuture<OperationResult> setThresholdSigPubKey(Session session, String scope, String table, ThresholdSigOptions options) {
        try {
            ...
            logger.error("Failed set threshold sig public key", e);
            return failure(e);
        }
    }
    ...
    public CompletableFuture<OperationResult> readThresholdSigPubKey(Session session, String scope, String table, ThresholdSigOptions options) {
        try {
            ...
            logger.error("Failed read threshold sig public key", e);
            return failure(e);
        }
    }
